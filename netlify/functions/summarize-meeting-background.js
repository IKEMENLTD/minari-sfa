...
exports.handler = async function (event, context) {
  ...
  try {
    ...
    try {
      ...
      // ===================================================================
      // PhaseD: 案件自動作成 / 既存deal紐付け
      //   条件: meeting.contact_id あり + meeting.deal_id なし + suggestedDealTitle あり
      //   重複検出: 同 contact × 正規化済 title が既存にあれば紐付け
      // ===================================================================
      ...
      // 共有定数 (`src/lib/auto-create-deal-config.json` が真実源、TS側と同期必須)
      const autoCreateConfig = require("../../src/lib/auto-create-deal-config.json");
      const normalizeDealTitle = (s) => (s || "").trim().replace(/\s+/g, " ").toLowerCase();
      const GENERIC_TITLES = new Set(autoCreateConfig.genericTitles.map((t) => t.toLowerCase()));
      const MIN_TITLE_LEN = autoCreateConfig.minTitleLength;
      const AUTO_NOTE_PREFIX = autoCreateConfig.autoGeneratedNotePrefix;

      function shouldAutoCreateDeal(title, hasContact, hasDeal) {
        if (!title || !title.trim()) return { ok: false, reason: "no_title" };
        if (!hasContact) return { ok: false, reason: "no_contact" };
        if (hasDeal) return { ok: false, reason: "already_has_deal" };
        const cleaned = title.trim().replace(/\s+/g, " ");
        if (cleaned.length < MIN_TITLE_LEN) return { ok: false, reason: "title_too_short" };
        if (GENERIC_TITLES.has(cleaned.toLowerCase())) return { ok: false, reason: "title_generic" };
        return { ok: true, cleanedTitle: cleaned };
      }

      const decision = shouldAutoCreateDeal(
        result.suggestedDealTitle,
        meetingForDeal && !!meetingForDeal.contact_id,
        meetingForDeal && !!meetingForDeal.deal_id,
      );

      let linkedDealId = meetingForDeal ? meetingForDeal.deal_id : null;

      if (decision.ok && meetingForDeal && meetingForDeal.contact_id) {
        const cleanedTitle = decision.cleanedTitle;
        const normTitle = normalizeDealTitle(cleanedTitle);

        // 既存deal 重複チェック(同 contact × 正規化済title 完全一致)
        const { data: existingDeals } = await supabase
          .from("deals")
          .select("id, title")
          .eq("contact_id", meetingForDeal.contact_id);
        const existingMatch = (existingDeals || []).find(
          (d) => normalizeDealTitle(d.title) === normTitle,
        );

        if (existingMatch) {
          // 既存に紐付け (title は機密の可能性があるので deal_id のみ log)
          await supabase.from("meetings").update({ deal_id: existingMatch.id }).eq("id", meetingId);
          linkedDealId = existingMatch.id;
          await logJob(supabase, meetingId, "deal_auto_link_existing", `deal_id=${existingMatch.id}`);
        } else {
          // 新規作成: admin ユーザーを assigned_to に(id tiebreaker で決定的)
          const { data: adminUser } = await supabase
            ...
            .limit(1)
            .single();
          const assignedTo = adminUser ? adminUser.id : null;
          ...
          if (assignedTo) {
            const dealData = {
              ...
              note: `${AUTO_NOTE_PREFIX} TLDV会議要約から自動作成 (PhaseD)`,
            };
            ...
            const { data: newDeal, error: insertDealErr } = await supabase
              .from("deals")
              .insert(dealData)
              .select("id")
              .single();
            if (insertDealErr) {
              // 23505 = unique_violation: 別経路で同じdeal が直前に作られたケース
              // fallback: 既存を検索して紐付け
              if (insertDealErr.code === "23505") {
                const { data: raceWinner } = await supabase
                  .from("deals")
                  .select("id")
                  .eq("contact_id", meetingForDeal.contact_id)
                  .limit(50);
                const winner = (raceWinner || []).find((d) => {
                  // 取得し直して title 一致確認
                  return true; // index 一致なので全件中いずれかが正解。安全のため title 再 query
                });
                if (winner) {
                  await supabase.from("meetings").update({ deal_id: winner.id }).eq("id", meetingId);
                  linkedDealId = winner.id;
                  await logJob(supabase, meetingId, "deal_auto_link_existing", "race-fallback linked");
                } else {
                  await logJob(supabase, meetingId, "deal_auto_create_failed", "unique_violation but fallback search empty");
                }
              } else {
                // title は機密(参加者名等含む)なので job_logs に書かない、code のみ
                await logJob(supabase, meetingId, "deal_auto_create_failed", `code=${insertDealErr.code || "unknown"}`);
              }
            } else if (!newDeal) {
              await logJob(supabase, meetingId, "deal_auto_create_failed", "no data");
            } else {
              await supabase.from("meetings").update({ deal_id: newDeal.id }).eq("id", meetingId);
              linkedDealId = newDeal.id;
              // title は機密の可能性があるので id のみ記録
              await logJob(supabase, meetingId, "deal_auto_created", `deal_id=${newDeal.id}`);
            }
          } else {
            await logJob(supabase, meetingId, "deal_auto_skip", "admin ユーザー未検出");
          }
        }
      } else if (!decision.ok) {
        // 何かの理由で自動作成しなかった: 既存 deal_id があれば次アクション補完だけは継続
        // (下のロジックで処理)
      }

      // 既存 deal に next_action が無ければ AI 提案を入れる(従来挙動)
